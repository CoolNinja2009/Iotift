...
NEWLINE tokens are used only for line tracking and are NOT emitted.
"""
...
TT = type('TT', (), {
    ...
    # math functions — kept as distinct token types so parser can identify them
    'SIN'   : 'SIN',
    'COS'   : 'COS',
    'TAN'   : 'TAN',
    'SQRT'  : 'SQRT',
...
# Math function names → their token type. Defined here (single source of truth).
MATH_KEYWORDS = {
    'sin'  : TT.SIN,
    'cos'  : TT.COS,
    'tan'  : TT.TAN,
    ...
    while pos < len(source):
        ...
        if source[pos] == 'c' and _is_word_boundary(pos + 1):
            ...
            if m:
                ...
                if brace_pos < len(source) and source[brace_pos] == '{':
                    ...
                    tokens.append(Token(TT.C_BLOCK, (scope, raw_code), start_line, start_col))
                    pos        = scan_pos
                    line       = scan_line
        ...
        for ttype, pat in TOKEN_PATTERNS:
            ...
            if ttype in ('SKIP', 'COMMENT'):
                ...
                # Only used for line tracking — NOT emitted into token stream
                line      += 1
                line_start = pos + 1
                ...
                tokens.append(Token(TT.STR_LIT, text[1:-1], line, col))

            elif ttype == 'IDENT':
                if text in ('true', 'false'):
                    tokens.append(Token(TT.BOOL_LIT, text == 'true', line, col))
                elif text in MATH_KEYWORDS:
                    tokens.append(Token(MATH_KEYWORDS[text], text, line, col))
        ...
        if not matched:
            raise LexError(
                f"Unexpected character {source[pos]!r} at line {line} col {pos - line_start + 1}"
            )
