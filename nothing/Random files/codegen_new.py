...
class CodeGen:
    def __init__(self, device: str = 'esp32'):
        ...
        self.indent_level = 0
        self.pins = {}
        self.pwm_pins = {}         # {pin_name: {'number': int, 'channel': int, 'freq': int, 'resolution': int}}
        self.pwm_channel_counter = 0
        self.every_count = 0
        self.every_labels = {}
    ...
    def _collect(self, node: Node):
        ...
        if isinstance(node, PinDecl):
            ...
            # Track PWM pins and assign channels
            if node.direction == 'pwm':
                channel = self.pwm_channel_counter
                self.pwm_channel_counter += 1
        ...
        if isinstance(node, OnThreshold):
        ...
        if isinstance(node, ImportDecl):
            # Log warning for imports
            self._global_state.append(f"// import \"{node.path}\";")
            return
    ...
    def _emit_struct(self, node: StructDecl):
        """Emit struct definition in global section"""
        lines = [f"struct {node.name} {{"]
        for field in node.fields:
            ctype = {'int': 'int', 'float': 'float', 'bool': 'bool'}.get(field.vtype, field.vtype)
            lines.append(f"  {ctype} {field.name};")
        lines.append("};")
        self._global_state.extend(lines)

    def _emit_array_decl(self, node: ArrayDecl):
        """Emit array declaration in global section"""
    ...
    def _collect_fn_decl(self, node: FnDecl):
        ...
        params = ', '.join(f"{p.vtype} {p.name}" for p in node.params) if node.params else 'void'
        
        fn_lines = [f"{ret_type} {node.name}({params}) {{"] + body_lines + ["}"]
        self._user_fns.append(fn_lines)
    ...
    def _collect_on_threshold(self, node: OnThreshold):
        """Collect and emit OnThreshold as a handler"""
        fn_name = f"handle_threshold_{node.pin}"
        body_lines = []
        cond = self._expr_c(BinOp(left=Identifier(name=node.pin), op=node.op, right=node.value))
    ...
    def _collect_loop_block(self, node: LoopBlock):
        """Collect and emit LoopBlock as a handler"""
        fn_name = f"handle_loop"
        body_lines = []
        for stmt in node.body:
            body_lines.extend(self._stmt_lines(stmt, '    '))
        self._handler_fns.append(self._make_void_fn(fn_name, body_lines))
    ...
    def _emit_setup(self):
        ...
        for name, pin in self.pins.items():
            # Skip PWM pins in pinMode - they need ledcSetup/ledcAttachPin instead
        ...
        # PWM setup: ledcSetup(channel, freq, resolution) and ledcAttachPin(pin, channel)
        for pin_name, pwm_info in self.pwm_pins.items():
            channel = pwm_info['channel']
            freq = pwm_info['freq']
            resolution = pwm_info['resolution']
            pin_num = pwm_info['number']
            self.lines.append(f'  ledcSetup({channel}, {freq}, {resolution});')
            self.lines.append(f'  ledcAttachPin({pin_num}, {channel});')
    ...
    def _stmt_c(self, node: Node) -> str:
        if isinstance(node, PwmSetup):
            # LED.setup(freq, resolution) - store the values for setup phase
            if node.pin in self.pwm_pins:
                self.pwm_pins[node.pin]['freq'] = node.freq
                self.pwm_pins[node.pin]['resolution'] = node.resolution
            return f"// {node.pin}.setup({node.freq}, {node.resolution})"
        
        if isinstance(node, PwmWrite):
            # LED.write(value) - emit ledcWrite call
            if node.pin in self.pwm_pins:
                channel = self.pwm_pins[node.pin]['channel']
                val_expr = self._expr_c(node.value)
                return f"ledcWrite({channel}, (uint32_t){val_expr});"
            return f"// {node.pin}.write({self._expr_c(node.value)})"
        ...
        if isinstance(node, WhileStmt):
            return self._while_lines(node)[0]  # First line of while
        
        if isinstance(node, ForStmt):
            return self._for_lines(node)[0]  # First line of for
    ...
    def _for_lines(self, node: ForStmt, indent: str = '') -> List[str]:
        ...
        init_str = self._stmt_c(node.init) if node.init else ''
        cond_str = self._expr_c(node.condition) if node.condition else ''
        step_str = self._stmt_c(node.step).rstrip(';') if node.step else ''  # Remove semicolon for for loop
        
        lines = [f"{indent}for ({init_str}; {cond_str}; {step_str}) {{"]
        for stmt in node.body:
            lines.extend(self._stmt_lines(stmt, indent + '  '))
        lines.append(f"{indent}}}")
        return lines
    ...
    def _expr_c(self, node):
        ...
        if isinstance(node, MathExpr):
            ...
            args = ', '.join(self._expr_c(arg) for arg in node.args)
