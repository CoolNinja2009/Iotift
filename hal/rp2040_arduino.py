"""
RP2040 Arduino HAL — Raspberry Pi Pico via Arduino-Pico core.

Uses Earle Philhower's Arduino-Pico core (github.com/earlephilhower/arduino-pico)
which provides full RP2040 peripheral support over the Arduino API.
"""
...
class RP2040ArduinoHAL(HALBase):
    """HAL for RP2040 microcontrollers running the Arduino-Pico core."""

    @property
    def target_name(self) -> str:
        return 'RP2040 (Arduino-Pico)'
    ...
    def attach_interrupt(self, pin_expr: str, isr_name: str, mode: str) -> str:
        # RP2040 supports interrupts on all GPIOs via PIO/GPIO IRQ.
        return (
            f'attachInterrupt('
    ...
    # ── PWM (hardware PWM slice) ────────────────────────────────

    def pwm_setup(self, channel: int, freq: int, resolution: int) -> List[str]:
        # RP2040 has 8 PWM slices (16 outputs). The Arduino-Pico core
        # maps analogWrite() to hardware PWM with configurable wrap.
        return [f'analogWriteFreq({freq});  // PWM freq for channel {channel}',
                f'analogWriteRange((1U << {resolution}) - 1U);  // {resolution}-bit']

    def pwm_attach(self, pin: int, channel: int) -> str:
        return f'pinMode({pin}U, OUTPUT);  // PWM ready on GPIO {pin}'
    ...
    def isr_attribute(self) -> str:
        # RP2040 PIO/GPIO IRQs don't need special attributes.
        return ''

    # ── Debug ──────────────────────────────────────────────────────

    def breakpoint_instruction(self) -> str:
        # ARM Cortex-M0+ breakpoint
