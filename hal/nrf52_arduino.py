"""
nRF52 Arduino HAL — nRF52840/nRF52832 via Adafruit nRF52 core.

Supports Adafruit Feather nRF52840, Seeed XIAO BLE, and other nRF52
boards running the Adafruit Arduino-nRF52 core.

The nRF52 has built-in BLE; this HAL exposes standard Arduino peripherals.
BLE is handled via the production features API (stdlib/ble.iot).
"""
...
class NRF52ArduinoHAL(HALBase):
    """HAL for nRF52 microcontrollers running the Adafruit Arduino core."""

    @property
    def target_name(self) -> str:
        return 'nRF52 (Arduino)'
    ...
    def attach_interrupt(self, pin_expr: str, isr_name: str, mode: str) -> str:
        # nRF52 uses GPIOTE for pin interrupts — all pins supported.
        return (
            f'attachInterrupt('
    ...
    # ── PWM (hardware PWM via nRF52 PWM peripheral) ─────────────

    def pwm_setup(self, channel: int, freq: int, resolution: int) -> List[str]:
        # nRF52 has a hardware PWM peripheral with up to 4 channels.
        # The Adafruit core maps analogWrite() to hardware PWM.
        return [f'// nRF52 PWM channel {channel}: freq={freq}Hz, res={resolution}-bit']
    ...
    # ── I2C (Wire — nRF52 uses TWIM) ───────────────────────────────

    def i2c_begin(self, sda: int, scl: int, speed_hz: int = 100000) -> List[str]:
        lines = [f'Wire.begin();']
        # On nRF52, SDA/SCL are fixed per board variant — Wire.begin()
        # uses the default pins. For custom pins, use Wire.setPins().
        if sda or scl:
            lines = [f'Wire.setPins({sda if sda else "PIN_WIRE_SDA"}, '
                     f'{scl if scl else "PIN_WIRE_SCL"});',
                     f'Wire.begin();']
        if speed_hz != 100000:
    ...
    def analog_set_resolution(self, bits: int) -> str:
        # nRF52 ADC defaults to 14-bit with oversampling.
        return f'analogReadResolution({bits});'

    # ── ISR ──────────────────────────────────────────────────────

    def isr_attribute(self) -> str:
        # nRF52 uses SD_EVT_IRQHandler / GPIOTE_IRQHandler — no special attr.
        return ''

    # ── Debug ──────────────────────────────────────────────────────

    def breakpoint_instruction(self) -> str:
        # ARM Cortex-M4 breakpoint
